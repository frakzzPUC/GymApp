...
export function TrainingDietDashboard({
  userProfile,
  onMarkComplete,
}: TrainingDietDashboardProps) {
  const todaysActivities = getTodaysActivities(userProfile);
  const today = new Date();

  // Estados para controlar progresso interativo
  const [hasCompletedTodaysWorkout, setHasCompletedTodaysWorkout] = useState(
    hasCompletedWorkout(today, userProfile)
  );
  ...
  const [activeMinutesToday, setActiveMinutesToday] = useState(
    userProfile.progress?.weeklyActivity || 0
  );
  const [totalCaloriesConsumed, setTotalCaloriesConsumed] = useState(
    userProfile.progress?.caloriesConsumed || 0
  );
  const [completedWorkoutsWeek, setCompletedWorkoutsWeek] = useState(
    userProfile.progress?.completedExercises || 0
  );
  ...
  const [completedMealIds, setCompletedMealIds] = useState<Set<number>>(
    new Set()
  );
  ...
  useEffect(() => {
    ...
    const calories = mealPlan
      ...
      .reduce((total, meal) => {
        const calorieValue = meal.calories
          ? parseInt(meal.calories.replace(/\D/g, ""))
          : 0;
  ...
  const dailyCaloriesGoal =
    mealPlan.reduce((total, meal) => {
      const calorieValue = meal.calories
        ? parseInt(meal.calories.replace(/\D/g, ""))
        : 0;
      return total + calorieValue;
    }, 0) ||
    userProfile.progress?.caloriesGoal ||
    2000;
  ...
  const stats = {
    ...
    weeklyProgress: Math.min(
      100,
      Math.round((completedWorkoutsWeek / (userProfile.daysPerWeek || 3)) * 100)
    ),
  };
  ...
  const handleWorkoutComplete = async (workoutId: string) => {
    try {
      ...
      setCompletedWorkoutsWeek((prev) =>
        Math.min(prev + 1, userProfile.daysPerWeek || 3)
      );
  ...
  const handleQuickMealLog = () => {
    ...
    const nextMealIndex = mealPlan.findIndex(
      (_, index) => !completedMealIds.has(index)
    );
    if (nextMealIndex !== -1) {
    ...
    <div className="space-y-6">
      ...
      <NutritionPlanModal
        ...
        nutritionPlan={
          nutritionPlan ||
          userProfile.aiNutritionPlan ||
          "Nenhum plano nutricional disponível. Gere seus planos personalizados na seção 'Planos de IA' do menu."
        }
