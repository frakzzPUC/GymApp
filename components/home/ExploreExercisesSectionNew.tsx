...
import { Search, TrendingUp, Clock, Target, Dumbbell, ChevronRight, Filter, Loader2, Play, Info, Zap, CheckCircle } from "lucide-react"
...
export default function ExploreExercisesSection() {
  ...
  React.useEffect(() => {
    console.log('🎭 Componente: exercises mudou para:', exercises?.length || 0, 'exercícios');
  ...
  const safeCategories = Array.isArray(categories) ? categories : ['chest', 'back', 'shoulders', 'arms', 'legs', 'abs'];
  ...
  const displayedCategories = showAllCategories ? categoryItems : categoryItems.slice(0, 8)
  ...
  return (
    <section id="exercises" className="w-full py-12 md:py-24 lg:py-32 bg-gray-50 dark:bg-gray-900/50">
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
          <div className="space-y-2">
            ...
            <p className="max-w-[900px] text-muted-foreground md:text-xl">
              Descubra exercícios profissionais da ExerciseDB com mais de 1300+ opções, GIFs animados e instruções detalhadas.
            </p>
          </div>
        </div>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {displayedCategories.map((category: any) => (
                <Badge
                  key={category.value}
                  variant={selectedBodyPart === category.value ? "default" : "secondary"}
          ...
          <div className="mb-6 p-4 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-800">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">
                  Exercícios para: <strong>{BODY_PARTS_TRANSLATION[selectedBodyPart] || selectedBodyPart}</strong>
                </span>
              </div>
              <Button 
                variant="ghost" 
        ...
        {loading && (
          <div className="flex justify-center items-center py-12">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground">Carregando exercícios da ExerciseDB API...</p>
              <p className="text-xs text-gray-500">Verifique o console do navegador para logs de debug</p>
            </div>
          </div>
        )}
        ...
        {/* Empty state */}
        {!loading && (!Array.isArray(exercises) || exercises.length === 0) && !error && (
          <div className="text-center py-12">
            <div className="space-y-4">
              <div className="mx-auto w-12 h-12 bg-muted rounded-full flex items-center justify-center">
                <Search className="h-6 w-6 text-muted-foreground" />
              </div>
              <h3 className="text-lg font-semibold">Nenhum exercício carregado</h3>
              <p className="text-muted-foreground max-w-md mx-auto">
                A API da ExerciseDB não retornou exercícios. Verifique o console do navegador para mais detalhes.
              </p>
              ...
              <p className="text-xs text-gray-500 mt-2">
                Estado atual: exercises={Array.isArray(exercises) ? `Array[${exercises.length}]` : typeof exercises}, loading={loading.toString()}, error={error || 'null'}
              </p>
            </div>
          </div>
        )}
        ...
        {Array.isArray(exercises) && exercises.length > 0 && exercises.length >= 20 && (
          <div className="text-center mt-8">
            <Button 
              onClick={async () => await getAllExercises((Array.isArray(exercises) ? exercises.length : 0) + 20)}
              disabled={loading}
              variant="outline"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {selectedExercise.targetMuscle && (
                  <div className="bg-muted/50 rounded-lg p-4 text-center">
                    ...
                    <div className="text-sm text-muted-foreground">{selectedExercise.targetMuscle}</div>
                  </div>
                )}
                {selectedExercise.equipment && (
                  <div className="bg-muted/50 rounded-lg p-4 text-center">
                    ...
                    <div className="text-sm text-muted-foreground">{selectedExercise.equipment}</div>
                  </div>
                )}
                {selectedExercise.bodyPart && (
                  <div className="bg-muted/50 rounded-lg p-4 text-center">
                    ...
                    <div className="text-sm text-muted-foreground">{selectedExercise.bodyPart}</div>
                  </div>
                )}
                ...
                <div className="bg-muted/50 rounded-lg p-4 text-center">
                  ...
                  <div className="text-sm text-muted-foreground">{selectedExercise.id}</div>
                </div>
              </div>
